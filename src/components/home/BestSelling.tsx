...
const BEST_SELLING_PRODUCTS = products.filter((product) => product.section === "best-selling").slice(0, 12);
   ...
   <div className="max-w-[95%] mx-auto px-12 relative">
    <div className="mb-8">
     ...
     <div className="flex items-center justify-between">
      <h2 className="text-3xl md:text-4xl font-black text-black tracking-tight">Best Selling Products</h2>
    ...
    <div
     ...
     className="flex gap-8 overflow-x-auto no-scrollbar pb-4 snap-x snap-mandatory"
    >
     {BEST_SELLING_PRODUCTS.map((product) => (
      <div key={product.id} className="group flex flex-col min-w-[calc(100%)] sm:min-w-[calc(50%-16px)] lg:min-w-[calc(25%-24px)] snap-start bg-white p-3 rounded-2xl hover:shadow-xl transition-all duration-300 border border-transparent hover:border-gray-100">
       <div
        className="relative bg-[#F5F5F5] rounded-xl mb-4 h-[260px] flex items-center justify-center overflow-hidden transition-all duration-500"
       >
        ...
        <div className="absolute top-3 right-3 flex flex-col gap-2 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
         ...
         <button
          ...
          className="w-10 h-10 rounded-full bg-white text-black flex items-center justify-center hover:bg-[#1443C3] hover:text-white transition-all shadow-md"
         >
       ...
       <div className="px-1 flex flex-col flex-grow">
        <h3 className="font-extrabold text-lg mb-1 text-[#1a1a1a] group-hover:text-[#1443C3] transition-colors">{product.name}</h3>
